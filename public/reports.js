...
const icon = (p) =>
  `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${p}</svg>`;
const ICONS = {
  check:
...
function gridlines(max, fmt) {
  ...
  steps.forEach((s) => {
    ...
    g.push(
      `<line x1="${CH.l}" y1="${y.toFixed(1)}" x2="${CH.w - CH.r}" y2="${y.toFixed(1)}" stroke="var(--track)" stroke-width="1"/>` +
...
function barChart(values, labels) {
  if (!values.length || values.every((v) => v === 0 && values.length < 2))
    return values.length
      ? barChartDraw(values, labels)
      : emptyChart("No periods recorded yet");
...
function barChartDraw(values, labels) {
  ...
  values.forEach((v, i) => {
    ...
    const x = CH.l + i * step + (step - bw) / 2,
      y = base - bh;
    bars +=
      `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" rx="3" fill="var(--accent)" opacity="${0.55 + (0.45 * (i + 1)) / n}"/>` +
      `<text x="${(x + bw / 2).toFixed(1)}" y="${(y - 4).toFixed(1)}" text-anchor="middle" font-size="9" font-weight="600" fill="var(--dim)">${v}%</text>` +
      (labels && labels[i]
        ? `<text x="${(x + bw / 2).toFixed(1)}" y="${base + 13}" text-anchor="middle" font-size="8.5" fill="var(--muted)">${labels[i]}</text>`
        : "");
  });
...
function lineChart(values, labels) {
  ...
  const pts = values.map((v, i) => [
    CH.l + (values.length > 1 ? i * step : innerW / 2),
    base - (v / max) * innerH,
  ]);
  const poly = pts
  ...
  const dots = pts
    .map(
      (p, i) =>
        ...
        (labels && labels[i]
          ? `<text x="${p[0].toFixed(1)}" y="${base + 13}" text-anchor="middle" font-size="8.5" fill="var(--muted)">${labels[i]}</text>`
          : ""),
    )
    .join("");
...
function ratingColors(r) {
  if (r === "Excellent")
    return ["var(--green-bg)", "var(--green-fg)", "#22a06b"];
...
async function connect() {
  try {
    ...
    showState(
      `<h2>Couldn't connect</h2><div>${e.message || "Authorization was cancelled."}</div><button id="cbtn">Try again</button>`,
    );
...
function renderDashboard() {
  ...
  const rows = d.history.length
    ? d.history
        ...
        .join("")
    : `<tr><td colspan="6" class="empty-cell">No stored periods yet — this ${mode === "monthly" ? "month" : "week"} is being recorded now and history will build over time.</td></tr>`;
  ...
  document.getElementById("fmtBtn").onclick = (e) => {
    e.stopPropagation();
    fmtMenu.hidden = !fmtMenu.hidden;
  };
  fmtMenu.querySelectorAll("button").forEach(
    (b) =>
      (b.onclick = (e) => {
        e.stopPropagation();
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest("#fmtSelect")) fmtMenu.hidden = true;
  });
...
const POP_TITLES = {
  active: "Active cards",
  achieved: "Completed cards",
  hours: "Hours tracked",
  overtime: "Cards over estimate",
};
function closePopover() {
  const p = document.querySelector(".popover");
  if (p) p.remove();
  document
    .querySelectorAll(".metric.open")
...
function togglePopover(el) {
  ...
  pop.style.left =
    Math.max(half, Math.min(center, grid.clientWidth - half)) + "px";
  el.classList.add("open");
  pop.querySelector(".pclose").onclick = (e) => {
    e.stopPropagation();
    closePopover();
  };
...
document.addEventListener("click", (e) => {
  if (!e.target.closest(".metric") && !e.target.closest(".popover"))
    closePopover();
});
...
function exportAs(fmt) {
  ...
  if (fmt === "csv") {
    ...
    const lines = report.history.map((r) =>
...
async function load() {
  ...
  let res;
  try {
    res = await ProgressReport.build(t, mode);
  } catch (e) {
    showState(
      `<h2>Something went wrong</h2><div>${e.message || e}</div><button id="rbtn">Retry</button>`,
    );
  ...
  if (res.needsAuth) {
    showState(
      `<h2>Connect Trello to load your report</h2><div>Reports read your board's cards to build metrics.</div><button id="cbtn">Connect Trello</button>`,
    );
    const b = document.getElementById("cbtn");
    if (b) b.onclick = connect;
    return;
  ...
  if (res.error) {
    showState(
      `<h2>Couldn't load board data</h2><div>${res.error}</div><button id="rbtn">Retry</button>`,
    );
    const b = document.getElementById("rbtn");
    if (b) b.onclick = load;
    return;
