...
function showBillingTab() {
  qs("popupFooter").style.display = "none";
  qs("viewContainer").innerHTML = `
    <div class="billing-view">
      ...
      <p>Manage your plan and members. Coming soon.</p>
    </div>`;
  setTimeout(() => t.sizeTo(document.body), 40);
}
...
function setActiveTab(tab) {
  ...
  else if (tab === "billing") showBillingTab();
}

function bindTabs() {
  qs("tabMapping").addEventListener("click", () => setActiveTab("mapping"));
  qs("tabBilling").addEventListener("click", () => setActiveTab("billing"));
  // Reports is a launcher, not an in-place tab — it opens the full dashboard modal.
  // The active tab (Mapping/Billing) stays visually selected underneath.
