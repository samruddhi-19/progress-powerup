...
function getLabelColor(color) {
  const map = {
    ...
    yellow: "#f5cd47", orange: "#fea362", red: "#f87168",
    pink: "#e774bb", purple: "#9f8fef",
  };
...
function render() {
  ...
  const labelTxt   = hasLabel
    ? cardMeta.labelName.length > 16 ? cardMeta.labelName.slice(0, 16) + "…" : cardMeta.labelName
    : "";
...
function bindEvents() {
  ...
  document.querySelectorAll(".view-btn").forEach((btn) => {
    btn.addEventListener("click", function () {
      state.logView = this.dataset.view; save(); render();
    });
  });
  ...
  document.querySelectorAll(".session-edit-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      function saveLabel() {
        ...
        if (state.history[idx]) { state.history[idx].label = newLabel; t.set("card", "shared", state); }
        render();
      }
