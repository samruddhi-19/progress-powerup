...
const icon = (p) =>
  `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${p}</svg>`;
const ICONS = {
  receipt: '<path d="M6 3h12v18l-3-2-3 2-3-2-3 2z"/><path d="M9 8h6M9 12h6"/>',
  clock: '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',
  dollar:
    '<path d="M12 2v20M17 6.5c0-1.9-2.2-3.5-5-3.5S7 4.6 7 6.5 9.2 10 12 10s5 1.4 5 3.5-2.2 3.5-5 3.5-5-1.6-5-3.5"/>',
...
function dueBadge(ds) {
  ...
  const dateLine = ds.date
    ? `<span class="due-date">Due ${fmtDate(ds.date)}</span>`
    : "";
...
function progressBar(pct) {
  ...
  const grad =
    p >= 100
      ? "linear-gradient(90deg,#37b57f,#4bce97)"
      : p >= 50
        ? "linear-gradient(90deg,#579dff,#85b8ff)"
...
function renderDashboard() {
  ...
  const detailRows = cardDetails.length
    ? cardDetails
        .map(
          (c) => `
    <tr>
      ...
      <td class="r">${dueCell(c.due)}</td>
        ...
        .join("")
    : `<tr><td colspan="4" class="empty-cell">No cards are mapped for tracking yet.</td></tr>`;

  const billRows = billableList.length
    ? billableList
        .map(
          (c) => `
    <tr>
      ...
      <td class="r amt">$${money(c.amount, 2)}</td>
        ...
        .join("")
    : `<tr><td colspan="4" class="empty-cell">No cards have an hourly rate set yet.</td></tr>`;
...
function exportCSV() {
  if (!report) return;
  const cardDetails = Array.isArray(report.cardDetails)
    ? report.cardDetails
    : [];
  ...
  lines.push(
    ["Task name", "List", "Completion (%)", "Hours taken", "Due status"]
      .map(q)
      .join(","),
  );
  cardDetails.forEach((c) =>
    lines.push(
      [c.name, c.list, c.progress, c.hours, c.due ? c.due.label : ""]
        .map(q)
        .join(","),
    ),
  );
  ...
  lines.push(
    ["Work", "List", "Hours", "Hourly rate ($)", "Total amount ($)"]
      .map(q)
      .join(","),
  );
  billableList.forEach((c) =>
    lines.push(
  ...
  if (billableList.length) {
    lines.push(
      ["Total", "", metrics.billableHours, "", metrics.totalAmount.toFixed(2)]
        .map(q)
        .join(","),
    );
  }
  ...
  a.href = url;
  a.download = `progress-invoice-${stamp}.csv`;
  a.click();
...
async function load() {
  ...
  if (res.needsAuth) {
    showState(
      `<h2>Connect Trello to load billing</h2><div>Billing reads your board's cards to compute rates and hours.</div><button id="cbtn">Connect Trello</button>`,
    );
    const b = document.getElementById("cbtn");
    if (b) b.onclick = connect;
    return;
