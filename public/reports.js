/* global TrelloPowerUp, ProgressReport */
      ...
      const ICONS = {
        ...
        sun:'<circle cx="12" cy="12" r="4"/><path d="M12 3v1M12 20v1M4.9 4.9l.7.7M18.4 18.4l.7.7M3 12h1M20 12h1M4.9 19.1l.7-.7M18.4 5.6l.7-.7"/>',
      ...
      function renderDashboard(){
        ...
        app().innerHTML=`
          <div class="topbar"><h1>History reports &amp; export</h1>
            <div class="actions">
              ...
              <button class="theme-toggle" id="themeToggle" aria-label="Toggle theme" title="Toggle light / dark theme">${icon(theme==="light"?ICONS.moon:ICONS.sun)}</button>
            </div>
          </div>
        ...
        document.getElementById("themeToggle").onclick=()=>{
          ...
          applyTheme();
          try{t.set("member","private","reportTheme",theme);}catch(e){}
        };
      ...
      function applyTheme(){
        ...
        // repaint just the toggle icon if it's on-screen
        const btn=document.getElementById("themeToggle");
        if(btn) btn.innerHTML=icon(theme==="light"?ICONS.moon:ICONS.sun);
      }
      // Load user's saved theme (Trello per-member private storage), else default dark.
      Promise.resolve()
        .then(()=>t.get("member","private","reportTheme"))
        .then(saved=>{ theme=(saved==="light"||saved==="dark")?saved:"dark"; })
        .catch(()=>{ theme="dark"; })
        .finally(()=>{ applyTheme(); load(); });